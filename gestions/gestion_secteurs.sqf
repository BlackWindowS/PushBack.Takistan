...
_secteurs = missionnamespace getvariable ["BIS_fnc_moduleSector_sectors",[]];

BwS_capture_possible = false;
BwS_capture_possible_force = false;

while {true} do
{
	if (((({side _x == east} count allplayers) > 3) && (({side _x == west} count allplayers) > 3)) || BwS_capture_possible_force) then {
		BwS_capture_possible = true;
	}
	else {
		BwS_capture_possible = false;
	};
	...
	{
		if (BwS_capture_possible) then
		{
			_x setVariable ["sides", [WEST, EAST], true];
		} else {
			_x setVariable ["sides", [(_x getVariable "owner")], true];
		};
	} forEach _secteurs;
...
//_secteurs = missionnamespace getvariable ["BIS_fnc_moduleSector_sectors",[]];
// {
	// _x setVariable ["costplayers", 0.5, true];
	// _x setVariable ["costinfantry", 0.5, true];
	// _x setVariable ["costair", (-0.5), true];
// } forEach _secteurs;
// ["taskowner","costwheeled","bis_fnc_moduleexecute_activate","finalized","pos","tasks","onownerchange","designation","costair","costwater","bis_fnc_moduleinit_iscuratorplaced","step","bis_fnc_moduleinit_status","defaultowner","ownerlimit","bis_fnc_initmodules_activate","owner","bis_fnc_initmodules_priroty","costtracked","sidescore","costplayers","contested","areas","flags","scorereward","name","costinfantry","taskdescription","#var","sides","tasktitle","bis_fnc_addscriptedeventhandler_ownerchanged"]
/*
/*
