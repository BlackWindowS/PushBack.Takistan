...
_ecran setObjectTextureGlobal [0, format ["#(argb,512,512,1)r2t(%1,1.0)", _nomDuStream]];
_cam = "camera" createVehicle [0, 0, 0];
...
},[_ecran],-90,false,true,"","(_target getVariable ""controller"") isEqualTo objNull"];
...
}, [_ecran],-90,false,true,"","!((_target getVariable ""mode"") == 1)"];
...
},[_ecran, _nomDuStream],-90,false,true,"","((_target getVariable ""mode"") == 1)"];
...
while {true} do {
	_cam setVectorDirAndUp [(eyeDirection (_ecran getVariable "controller")), vectorUp (_ecran getVariable "controller")];
	_ecran setObjectTextureGlobal [0, format ["#(argb,512,512,1)r2t(%1,1.0)", _nomDuStream]];
	_eyepos = ASLtoATL eyePos (_ecran getVariable "controller");
