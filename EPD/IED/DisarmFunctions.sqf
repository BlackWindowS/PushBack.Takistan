...
DISARM_ADD_ACTION = {
	...
	_itemRequirement = _itemRequirement + "&& ""ACE_DefusalKit"" in (items player) && ""MineDetector"" in (items player)";
	
	(_iedArray select 0) addAction [("<t color=""#27EE1F"">") + ("Désarmer l'IED") + "</t>", DISARM_ACTION, [_iedArray,[_sectionName, _iedName]], 10, false, true, "", format["(_target distance _this < 3) %1", _itemRequirement]];
};
...
DISARM_ACTION = {
	...
	_bonusAdded = false;
	{
		if((not _bonusAdded) and (((typeof player) isKindOf _x ) or joueurEOD)) then {
